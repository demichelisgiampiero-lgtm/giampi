...
const doc = new Document({
  ...
  title: 'Quadro normativo — Le reti di società di ingegneria negli appalti pubblici',
  description: 'Fonti europee e nazionali, giurisprudenza e criteri di partecipazione alle gare italiane',
  styles: {
    default: { document: { run: { font: F, size: 21, color: '222222' }, paragraph: { spacing: { line: 276 } } } },
    paragraphStyles: [
      { id: 'Heading1', name: 'Heading 1', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: F, size: 30, bold: true, color: NAVY } },
      { id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: F, size: 24, bold: true, color: NAVY } },
      { id: 'Heading3', name: 'Heading 3', basedOn: 'Normal', next: 'Normal', quickFormat: true, run: { font: F, size: 21, bold: true, color: ACCENT } }
    ]
  },
  numbering: { config: [
    { reference: 'bullets', levels: [{ level: 0, format: LevelFormat.BULLET, text: '•', alignment: AlignmentType.LEFT, style: { paragraph: { indent: { left: 340, hanging: 200 } } } }] },
  ...
  sections: [{
    properties: { page: {
      ...
      margin: { top: convertMillimetersToTwip(22), bottom: convertMillimetersToTwip(20), left: convertMillimetersToTwip(20), right: convertMillimetersToTwip(20) }
    } },
    headers: { default: new Header({ children: [new Paragraph({
      alignment: AlignmentType.RIGHT, spacing: { after: 60 },
      border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: BAND, space: 4 } },
      children: [new TextRun({ text: 'Rete Masterplan · Quadro normativo · Reti di ingegneria negli appalti · 15 settembre 2026', font: F, size: 16, color: GREY })]
    })] }) },
    footers: { default: new Footer({ children: [new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [
        new TextRun({ text: 'Ricognizione di fonti — non è un parere legale          ', font: F, size: 15, color: GREY }),
...
Packer.toBuffer(doc).then(buf => { fs.writeFileSync(process.argv[2] || 'normativa.docx', buf); console.log('OK', (buf.length/1024).toFixed(1), 'KB'); });
